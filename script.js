let materialLength = document.getElementById("materialLength");
...
function calculate() {
  ...
  for (let i = 0; i < inputs.length; i++) {
    for (let p = 0; p < inputs.length; p++) {
      sum.push({
        ...
        firstQuantity: inputs[i].quantity,
        secondNum: inputs[p].length,
        secondQuantity: inputs[p].quantity,
  ...
  let removeDuplicateSum = removeDuplicates(sum, "total");

  console.log(removeDuplicateSum);

  let filteredSum = removeDuplicateSum.filter(index => {
    return index.total <= materialLength.value;
  });
